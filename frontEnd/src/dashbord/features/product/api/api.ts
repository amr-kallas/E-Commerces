...
import { categoryBody } from '@features/category/api/type'
...
const API = {
  ...
  getAllCategories: async () => {
    const { data } = await axios.get<categoryBody[]>(
  ...
  get: async (id: string) => {
    const { data } = await axios.get<Product[]>(API_ROUTES.PRODUCT.GET(id))
    return data
  },
